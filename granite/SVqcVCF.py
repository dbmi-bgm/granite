...
def to_json(stat_dict, stat_to_add):
    ...
    for ID in stat_dict:
        ...
        for k, v in stat_dict[ID].items():
            tmp_dict = {}
            # total variants
            if k in ['del', 'dup', 'ins', 'inv', 'bnd', 'cnv']:
                tmp_total.setdefault(k.upper(), v['total'])
                tmp_total['total'] += v['total']
            #end if
            #heterozygosity ratio
            if k in ['del', 'dup', 'ins', 'inv', 'bnd', 'cnv'] and 'het_hom' in stat_to_add:
...
def main(args):
    ...
    for ID in ID_list:
        stat_dict.setdefault(ID, {
                            ...
                            'dup': {'het': 0, 'hom': 0, 'total': 0},
                            'ins': {'het': 0, 'hom': 0, 'total': 0},
                            'inv': {'het': 0, 'hom': 0, 'total': 0},
                            'bnd': {'het': 0, 'hom': 0, 'total': 0},
                            'cnv': {'het': 0, 'hom': 0, 'total': 0},
                            'sub': {
                                'A_G': 0, 'A_T': 0, 'A_C': 0,
                                'T_A': 0, 'T_G': 0, 'T_C': 0,
                                'C_A': 0, 'C_G': 0, 'C_T': 0,
